...
public class SpaceExplorer extends Thread {
	...
	public void run() {
		while(true) {
			Message headquartersMessage = channel.getMessageHeadQuarterChannel();
			if (headquartersMessage == null) {
				continue;
			} else if(headquartersMessage.getData().equals("EXIT") == true) {
				break;
			} else{
				String decodedFrequency;
				String data = headquartersMessage.getData();
				int parentSolarSystem = headquartersMessage.getParentSolarSystem();
				int currentSolarSystem = headquartersMessage.getCurrentSolarSystem();

				decodedFrequency = encryptMultipleTimes(data, hashCount);
				Message spaceExplorerMessage = new Message(parentSolarSystem, currentSolarSystem, decodedFrequency);
				channel.putMessageSpaceExplorerChannel(spaceExplorerMessage);
			}
		}
	}
